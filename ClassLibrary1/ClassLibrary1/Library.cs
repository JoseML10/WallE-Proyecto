...
namespace ClassLibrary1
{
    ...
    {
        void GetPrinteable(Entorno entorno);
    }
    ...
    public sealed class PointsFunction : Instruccion, Isecuenciable, IEnumerator<Point>
    {
        ...
        private Point InternalPoint(Figura figura )
        {
            if (figura is Circle c)
            {
                ...
                double radius = Math.Sqrt(random.NextDouble()) * c.Measure.Value * 4 / 5; // Ajuste aquí
    ...
    public class PrintExpression : Instruccion
    {
        ...
        public Instruccion Expression { get; }

        public PrintExpression(Instruccion expression)
        {
        ...
        public override object Evaluate(Entorno entorno)
        {
            ...
            Instruccion ToPrint =(Instruccion)Expression.Evaluate(entorno);

            if (ToPrint is IPrinteable Print_IPrinteable)
            {
                Print_IPrinteable.GetPrinteable(entorno);
                return null;
            }
            ...
            {
                throw new CustomException($"Can't print {ToPrint} type", CustomExceptionType.InvalidOperation);
            }
    ...
    public class Number : Instruccion, IPrinteable
{
        ...
        public void GetPrinteable(Entorno entorno)
        {

            PrintExpression.Printeables.Add(this.Value.ToString());
...
public sealed class CircleFunction : Function
{
    ...
    public override object Evaluate(Entorno entorno)
    {
        return new Circle((Point)entorno.BuscarVariable(P1.name).Value,(Number)Measure.Evaluate(entorno) );
    }
}
...
public sealed class Circle : Figura
{
    ...
    public Number Measure { get; }
    public Circle(string nombre)
    {
        Point p1 = new Point("p1");
        Centro = p1;
        Random r = new Random();
        Measure.Value = r.Next(1, 500);
        
        this.nombre = nombre;

    }

    public Circle(Point centro, Number measure)
    {
        Centro = centro;
        Measure = measure;
...
public class InfiniteSequence : Instruccion,IPrinteable, IEnumerator<Number>, Isecuenciable
{
    private int start;
    private int? end;
    private int current;
    ...
    public void GetPrinteable(Entorno entorno)
    {
        for (int i = 0; i < this.SecuenceCount; i++)
        {

            ((IPrinteable)this.GetElement(i, entorno)).GetPrinteable(entorno);
...
public class Cadena : Instruccion,IPrinteable
{
    ...
    public void GetPrinteable(Entorno entorno)
    {
        PrintExpression.Printeables.Add(cadena.ToString());
    }
}
...
public class SecuenceFigure<T> : Instruccion, IDrawable, IPrinteable ,Isecuenciable where T : Instruccion
{
...
public class IntersectExpression : Instruccion
{
    ...
    public override object Evaluate(Entorno entorno)
    {
        ...
        if (figure1.GetType() == typeof(Point) && figure2.GetType() == typeof(Circle) || figure1.GetType() == typeof(Circle) && figure2.GetType() == typeof(Point))
        {
            ...
            if (distance == Math.Abs(c.Measure.Value))
            {
                points.Add(p);
            }
        ...
        else if (figure1.GetType() == typeof(Line) && figure2.GetType() == typeof(Circle) || figure1.GetType() == typeof(Circle) && figure2.GetType() == typeof(Line))
        {
            ...
            points = IntersectCircleLine(c.Centro, c.Measure.Value, l.Punto1, l.Punto2);
            return new SecuenceFigure<Figura>(points);
        }
        //Ray, circulo
        ...
        else if (figure1.GetType() == typeof(Segment) && figure2.GetType() == typeof(Circle) || figure1.GetType() == typeof(Circle) && figure2.GetType() == typeof(Segment))
        {
            ...
            points = IntersectCircleLine(c.Centro, c.Measure.Value, s.Punto1, s.Punto2);
        ...
        else if (figure1.GetType() == typeof(Arc) && figure2.GetType() == typeof(Circle) || figure1.GetType() == typeof(Circle) && figure2.GetType() == typeof(Arc))
        {
            ...
            points = IntersectCircles(a1.Centro, a1.Measure, c.Centro, c.Measure.Value);
            if (points.Count == 1)
            {
                Point p = (Point)points[0];
        ...
        else if (figure1.GetType() == typeof(Circle) && figure2.GetType() == typeof(Circle))
        {
            ...
            points = IntersectCircles(c1.Centro, c1.Measure.Value, c2.Centro, c2.Measure.Value);
            if (points.Count != 0)
                return new SecuenceFigure<Figura>(points);
