...
namespace Wall_E
{
    ...
    public partial class Formulario :Form
    {
        ...
        public void DibujarCirculo(Graphics g, Circle c, string color, string etiqueta)
        {
            ...
            // Dibuja el círculo
            float x = (float)c.Centro.Coord.X - (float)c.Measure;
            float y = (float)c.Centro.Coord.Y - (float)c.Measure;
            int diameter = (int)c.Measure * 2;
            ...
            // Imprime el mensaje al lado del punto usando DrawString
            g.DrawString(etiqueta, drawFont, drawBrush, (float)c.Centro.Coord.X + (int)c.Measure * 2, (float)c.Centro.Coord.Y);
